...
export type EmailOwner = { id: string; label: string };
...
export function buildEmailDuplicateMap(
  ...
  for (const i of instructors) push(i.email, { id: i.id, label: i.name || "이름없음" });
  for (const ch of ytChannels) push(ch.email, { id: ch.id, label: `${ch.channel_name || "채널명없음"} (YT)` });
