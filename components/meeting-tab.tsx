    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-7 gap-2">
        {weekDays.map((day) => (
          <div
            key={day.label}
            className={`rounded-lg border p-2 min-h-[100px] ${isToday(day.date) ? "border-primary bg-primary/5" : "bg-card"}`}
          >
            <div className="flex items-center justify-between mb-1.5">
              <span className={`text-xs ${isToday(day.date) ? "text-primary font-bold" : "text-muted-foreground"}`}>{day.dayLabel}</span>
              <span className={`text-sm font-medium ${isToday(day.date) ? "text-primary" : ""}`}>{day.label}</span>
            </div>
              ...
              {day.meetings.map((m) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left rounded bg-blue-50 border border-blue-200 px-2 py-1 text-xs hover:bg-blue-100 transition-colors"
                  >
                    <div className="flex items-center justify-between">
      ...
      {/* 전체 리스트 */}
      <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">전체 미팅 목록</h3>
            ...
            <Card key={i.id} className="py-0 hover:border-primary/30 transition-colors">
              <CardContent className="p-4 space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  >{i.name}</span>
                  <Badge className={`text-xs px-2 py-0 ${STATUS_COLORS[i.status as InstructorStatus] || ""}`}>{i.status}</Badge>
                </div>
                ...
                <div className="flex gap-3">
                  {i.youtube && <a href={i.youtube} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline flex items-center gap-0.5">YT<ExternalLink className="h-3 w-3" /></a>}
                  {i.instagram && <a href={i.instagram} target="_blank" rel="noopener noreferrer" className="text-xs text-pink-600 hover:underline flex items-center gap-0.5">IG<ExternalLink className="h-3 w-3" /></a>}
                </div>
                {i.instructor_info && <p className="text-xs text-muted-foreground">{i.instructor_info}</p>}
                {i.meeting_memo && (
                ...
                <button onClick={() => setEditingMemo({ id: i.id, memo: i.meeting_memo || "" })} className="text-xs text-primary hover:underline">메모 수정</button>
              </CardContent>
            </Card>
          ))}
      </div>
